...
const HERO_URL = "https://images.unsplash.com/photo-1511818966892-d7d671e672a2?auto=format&fit=crop&w=1800&q=86";
...
const team = [
  { name: "Mina Shah", role: "Strategy + Growth", bio: "Finds the clearest route from ambition to action." },
  { name: "Theo Park", role: "Design Director", bio: "Makes complex ideas feel immediate and human." },
  { name: "Leena Kapoor", role: "Web + Technology", bio: "Builds the systems that make good ideas dependable." },
  { name: "Noah Williams", role: "Search + Content", bio: "Turns expertise into signals people can actually find." },
  { name: "Iris Chen", role: "Automation + Ops", bio: "Connects the moving pieces so growth has room to breathe." },
];
    ...
    <>
      ...
      <header className={`site-header glass-surface ${scrolled ? "site-header--scrolled" : ""}`}>
        ...
        <div className="header-actions">
          <MagneticLink href="#contact" className="header-cta glass-surface">Start a project</MagneticLink>
      ...
      <main id="top">
            ...
            <div className="hero-copy">
              ...
              <Reveal delay={0.3} y={18}>
                <div className="hero-actions">
                  <MagneticLink href="#contact" className="glass-surface">Start a project</MagneticLink>
        ...
        <section className="services-section section-pad" id="services">
          ...
          <div className="services-layout">
            ...
            <motion.div key={activeService} className={`service-panel glass-surface service-panel--${services[activeService].accent}`} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, ease: [0.23, 1, 0.32, 1] }}>
              ...
              <a href="#contact" className="service-panel__link">Talk to us about {activeService} <ArrowUpRight size={18} strokeWidth={1.5} /></a>
            </motion.div>
          </div>
        </section>
        ...
        <section className="about-section section-pad" id="about">
          ...
          <div className="about-body">
            ...
            <div className="team-list">{team.map((member, index) => <div className="team-row" key={member.name}><span className="team-row__index">0{index + 1}</span><div><h3>{member.name}</h3><p className="team-row__role">{member.role}</p></div><p className="team-row__bio">{member.bio}</p><a href="#contact" aria-label={`Connect with ${member.name}`}><MoveUpRight size={17} strokeWidth={1.5} /></a></div>)}</div>
          </div>
        </section>
        ...
        <section className="contact-section" id="contact">
          <div className="contact-section__inner section-pad"><SectionLabel number="08" light>Start a project</SectionLabel><div className="contact-copy"><h2>Ready to grow<br />your <em>business?</em></h2><p>Let’s build a digital presence that gets your business seen, trusted and chosen.</p><MagneticLink href="mailto:hello@synra.studio" dark className="glass-surface">Start a project</MagneticLink></div><div className="contact-bottom"><span>hello@synra.studio</span><span>Usually replies within 2 working days</span></div></div>
        </section>
      </main>
